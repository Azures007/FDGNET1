...
public interface OrderHeadRepository extends JpaRepository<TBusOrderHead,Integer>, JpaSpecificationExecutor<TBusOrderHead> {
    ...
    @Query(value = "select count(1) from t_bus_order_head a \n" +
            ...
            "join t_sys_process_info d2 on a2.process_id =d2.process_id\n" +
            ...
            "where a2.process_id=c.process_id and group_leader_id=?1 and TO_CHAR(body_plan_start_date,'YYYY-MM-DD') =?2 and a.is_deleted='1'", nativeQuery = true)
    Integer getCountCurrentTask(String userId, String currentDateStr);
...
//            "d2.process_id executeProcessId,\n" +
//            "d2.process_name executeProcessName,\n" +
//            "d2.process_number executeProcessNumber, \n" +
...
//            "where a2.process_id=a.current_process and group_leader_id=?1 and TO_CHAR(body_plan_start_date,'YYYY-MM-DD') =?2 and a.is_deleted='1'\n" +
//            "", nativeQuery = true)
//    List<Object[]> getTodayTaskList(String userId,
...
//                                    PageRequest sort);
    ...
    @Query(value = "select a.order_id as orderId,\n" +
            ...
            "join t_bus_order_process_lk a1 on a1.order_id=a.order_id \n" +
            "join t_bus_order_process a2 on a2.order_process_id=a1.order_process_id\n" +
            "join t_sys_process_info d on a2.process_id =d.process_id \n" +
            "join t_sys_personnel_info c on a2.person_id =c.personnel_id \n" +
            "left join t_sys_process_info g on g.process_id = a.current_process \n" +
            "where c.user_id =?1 and a2.process_status in (?2) and a.is_deleted='1'", nativeQuery = true)
    List<Object[]> getTaskListByProcessStatus(String userId, List<String> processStatus, PageRequest sort);
    ...
    @Query(value = "select count(1) from t_bus_order_head a \n" +
            ...
            "join t_sys_personnel_info c on a2.person_id =c.personnel_id \n" +
            "where c.user_id =?1 and a2.process_status in (?2) and a.is_deleted='1'", nativeQuery = true)
    Integer getTaskCountByProcessStatus(String userId, List<String> processStatus);

    @Query(value = "select process_status from t_bus_order_head a \n" +
            "join t_bus_order_process b on a.current_process =b.process_id \n" +
            "join t_sys_personnel_info c on b.person_id =c.personnel_id \n" +
            "where c.user_id =?1 and a.is_deleted='1'", nativeQuery = true)
    List<String> getTask(String userId);
    ...
    @Query(value = "select count(1) from t_bus_order_head a \n" +
            ...
            "join t_sys_process_class_rel e on d.process_id =e.process_id \n" +
            "join CLASS_GROUP_LEADER_VIEW f on e.class_id =f.class_id \n" +
            "where group_leader_id=?1 and a.order_id not in (\n" +
            "select a.order_id from t_bus_order_head a \n" +
            ...
            "where d.user_id =?1 and a.is_deleted='1'\n" +
            ")", nativeQuery = true)
    int getOffTask(String userId);
    ...
    @Query(value = "select a.order_id as orderId,\n" +
            ...
            "where d.user_id =?1 and a.is_deleted='1'\n" +
            ")", nativeQuery = true)
    List<Object[]> getOffTaskList(String userId, PageRequest sort);
...
//            "left join (select * from t_bus_order_process_record tbopr where bus_type ='PD') pd on a1.order_process_id=pd.order_process_id \n" +
            ...
            "",nativeQuery = true)
    List<Map> getTaskListByPersonIdAndProcessStatus(String userId, List<String> processStatus,String processNumber,String bodyLot, PageRequest sort);
    ...
    @Query(value = "select a.order_id as orderId,\n" +
            ...
            "",nativeQuery = true)
    List<Map> getTaskListByPersonIdAndProcessStatusAndOrderProcessType(String userId, List<String> processStatus, String orderProcessType, String processNumber, String bodyLot, PageRequest sort);
    ...
    @Query(value = "select COUNT(1)\n" +
            ...
            "and (a.body_lot=?4 or ?4='' or ?4 is null)" +
            "",nativeQuery = true)
    int getTaskListCountByPersonIdAndProcessStatus(String userId, List<String> processStatus,String processNumber,String bodyIot);
    ...
    @Query(value = "select count(1)\n" +
            ...
            "and (a.body_lot=?5 or ?5='' or ?5 is null)" +
            "",nativeQuery = true)
    int getTaskListCountByPersonIdAndProcessStatusAndOrderProcessType(String userId, List<String> processStatus, String orderProcessType, String processNumber, String bodyLot);
    ...
    @Query(value = "select sum(cout) from (\n" +
            ...
            "join t_bus_order_process c on b.order_process_id =c.order_process_id \n" +
            "join t_sys_process_info d on c.process_id =d.process_id \n" +
            "join t_sys_class_group_leader_rel e on c.class_id = e.class_id \n" +
            "join t_sys_personnel_info f on e.personnel_id =f.personnel_id \n" +
    ...
    int getWaitTaskCountByUserId2(String userId, String processNumber, String bodyLot);
    ...
    List<Map> getWaitTaskUserId2(String userId, String processNumber, String bodyLot, PageRequest of);
    ...
    @Query(value = "select sum(cout) from (\n" +
            ...
            "and (a.body_lot=?3 or ?3='' or ?3 is null)" +
            ") as t",nativeQuery = true)
    int getWaitTaskCountByUserIdHandOver(String userId,String processNumber,String bodyIot);
    ...
    @Query(value = "select a.order_id as orderId,\n" +
            ...
            "and (a.body_lot=?3 or ?3='' or ?3 is null)" +
            "",nativeQuery = true)
    List<Map> getWaitTaskUserIdHandOver(String userId, String processNumber, String bodyLot, PageRequest of);
    ...
    @Query(value = "select \n" +
            ...
            "and (a.body_lot=?3 or ?3='' or ?3 is null)" +
            "",nativeQuery = true)
    int getOffTask2(String userId,String processNumber,String bodyIot);
    ...
    @Query(value = "select \n" +
            ...
            "and (a.body_lot=?3 or ?3='' or ?3 is null)" +
            "",nativeQuery = true)
    List<Map> getOffTaskList2(String userId, String processNumber, String bodyLot, PageRequest of);
...
//            "d.process_number executeProcessNumber, \n" +
//            "c.process_status executeProcessStatus \n" +
//            ",c.record_type_pd recordTypePd \n" +
//            "from (select * from t_bus_order_head a left join t_sys_process_info m2 on a.current_process =m2.process_id ) as a\n" +
//            "join t_bus_order_process_lk b on a.order_id=b.order_id \n" +
//            "join  (select distinct a.*,b.record_type_pd from t_bus_order_process a left join (select * from t_bus_order_process_record where bus_type='PD') b on a.order_process_id =b.order_process_id  \n" +
//            ") c  on b.order_process_id =c.order_process_id \n" +
//            "join t_sys_process_info d on c.process_id =d.process_id \n" +
//            "join t_sys_class_group_leader_rel e on c.class_id = e.class_id \n" +
//            "join t_sys_personnel_info f on e.personnel_id =f.personnel_id \n" +
//            "where c.process_status='0' and c.type ='2' and a.is_deleted='1' and  f.user_id=?1\n" +
//            "and c.order_process_id not in (select order_process_id from  t_bus_order_process c  \n" +
//            "join t_sys_process_info d on c.process_id =d.process_id \n" +
//            "join t_sys_class_group_leader_rel e on c.class_id = e.class_id \n" +
//            "join t_sys_personnel_info f on e.personnel_id =f.personnel_id \n" +
//            "where f.user_id=?1 and type='2' and c.process_status='4' )",nativeQuery = true)
//    List<Map> getWaithandOverVerify(String userId, PageRequest of);
//
//    @Query(value = "select count(1)\n" +
//            "from (select * from t_bus_order_head a left join t_sys_process_info m2 on a.current_process =m2.process_id ) as a\n" +
//            "join t_bus_order_process_lk b on a.order_id=b.order_id \n" +
...
//            "join t_sys_class_group_leader_rel e on c.class_id = e.class_id \n" +
//            "join t_sys_personnel_info f on e.personnel_id =f.personnel_id \n" +
//            "where .user_id=?1 and type='2' and c.process_status='4' )",nativeQuery = true)
//    int countWaithandOverVerify(String userId);
    //根据订单号获取关联的订单信息
    @Query(value = "select  distinct a.order_id as orderId,\n" +
            "a.craft_id as craftId, \n" +
            "a.order_no as orderNo, \n" +
