...
public interface MenuRepository extends JpaRepository<TSysMenu,Integer>, JpaSpecificationExecutor<TSysMenu> {
    @Query(value = "select a.* from t_sys_menu a join t_sys_role_menu b on a.menu_id=b.menu_id where b.role_id=?1 and a.enabled='1' order by sort,menu_id asc",
            nativeQuery = true)
    List<TSysMenu> listMenuByRoleId(Integer roleId);

    @Query(value = "select a.path from t_sys_menu a join t_sys_role_menu b on a.menu_id=b.menu_id where b.role_id=?1 and a.enabled='1'",nativeQuery = true)
    ...
    @Query(value = "\n" +
            ...
            "where a.flag in (?2) and b.role_id =?1 and a.enabled ='1'",nativeQuery = true)
