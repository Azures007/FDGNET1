...
public class JpaUserDao extends JpaAbstractSearchTextDao<UserEntity, User> implements UserDao {
    ...
    @Override
    public User findByFirstName(TenantId tenantId, String lastName) {
        return DaoUtil.getData(userRepository.findByFirstName(lastName));
    }
    ...
    public PageData<User> findCustomerUsers(UUID tenantId, UUID customerId, PageLink pageLink) {
        Page<UserEntity> usersByAuthority = userRepository
                .findUsersByAuthority(
                        tenantId,
                        customerId,
                        Objects.toString(pageLink.getTextSearch(), ""),
    ...
    public void findAll() {
        ...
        List<User> ts = DaoUtil.convertDataList((List)all);
