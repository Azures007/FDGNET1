...
import { AddCheckCateComponent } from '../../components/quality/add-check-cate.component';


@Component({
  selector: 'tb-quality-check-cate',
  templateUrl: './quality-check-cate.html',
  styleUrls: ['./quality-cate.scss', './quality.component.scss']
})
export class QualityCheckCateComponent implements OnInit {
  searchData = {
    frequency: '',
    importantItem: '',
    current: 0,
  ...
  displayedColumns: string[] = ['no', 'frequency', 'importantItem', 'remark', 'enabled',
    'customColumn1',];
  ...
  searchList() {
    ...
    params = {
      ...
      body: {
        ...
        importantItem: this.searchData.importantItem,
      }
    }


    this.qualityService.fetchGetDailyList(params).subscribe(res => {
  ...
  handleEvent(data, type) {
    switch (type) {
      ...
      case 'disable':
        ...
        this.qualityService.fetchDisableDaily(params).subscribe(res => {
          if (res.errcode === 200) {
      ...
      case 'del':
        this.utils.confirm('温馨提示', `是否确认删除?`, () => {
          this.qualityService.fetchDelDaily(data.id).subscribe(res => {
            if (res.errcode === 200) {
  ...
  showAddVisibilly(data) {
    if (data.type) {
      this.qualityService.fetchGetDailyDetails(data.data.id).subscribe(res => {
        data.data = res.data;
        let dialogRef = this.dialog.open(AddCheckCateComponent, {
          width: "1400px",
          height: "auto",
        ...
        dialogRef.afterClosed().subscribe(result => {
          if (result && result === 'refresh') {
            this.searchList();
            const msg = data.type ? (data.type === 'details' ? '查看类目' : '编辑类目') : '新增类目';
